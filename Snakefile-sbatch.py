...
class SnakeJobSbatch(SnakeJob):
    def schedule(self):
        ...
        if self.rule == 'split1' or self.rule == 'split2':
            #hours = math.ceil(os.path.getsize(self.ifiles[0]) / float(500 * 1024 ** 2))
            hours = 1
            sbatch_cmd = 'sbatch -A b2010008 -p node -t %i:00:00 --output=snakemake-%%j.out -J %s ~/bin/sbatch_job bash %s' % (hours, self.rule, self.scriptname)
        else:
